...
| [examples/user-provided-sources-iraq-banking.md](examples/user-provided-sources-iraq-banking.md) | `user-provided sources` | Iraq banking-sector reform exposure for a correspondent bank (template form) |
| Sovereign wealth deployment, dark-fleet exposure, GCC correspondent banking | — | Deferred — see [examples/README.md](examples/README.md) |

`live-source-backed` examples are deferred from initial release; documented in `STATUS.md` and `examples/README.md` with reasons.
...
- **Live-source-backed examples (B2.1):** at least one Iran sanctions and one Hormuz/Red Sea example with primary OFAC and IMO sources, with retrieval dates and freshness discipline.
- **Validation script (B1.5):** structural validator for skill files, evidence-mode declarations and forbidden determinative claims.
...
- **Source-freshness practice (B2.6):** documented re-verification horizons in `docs/source-guide.md`, especially for OFAC designations and IMO maritime data.
- **External review (B2.2, B2.3, B2.7):** open to external reviewers from sanctions compliance, energy trading, shipping insurance, Gulf banking, or Iran-watcher analyst backgrounds.
